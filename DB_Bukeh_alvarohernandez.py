...
class VideoProcessorApp:
    ...
    def create_parameter_controls(self):
        ...
        # Banding Tab
        banding_frame = ttk.Frame(notebook)
        self.create_slider(banding_frame, "BANDING_INTENSITY", 0.0, 1.0, row=0)
        self.create_slider(banding_frame, "BAND_APPLY_PROB", 0.0, 1.0, row=1)
        notebook.add(banding_frame, text="Banding")
    ...
    def get_current_parameters(self):
        return {
            ...
            "BAND_APPLY_PROB": self.BAND_APPLY_PROB.get(),
    ...
    def get_parameters(self):
        return {
            ...
            "BANDING_INTENSITY": float(self.BANDING_INTENSITY.get()),

            "FRAME_SKIP_PROB": float(self.FRAME_SKIP_PROB.get()),
            "BAND_APPLY_PROB": float(self.BAND_APPLY_PROB.get()),
    ...
    def set_parameters(self, params):
        ...
        self.BANDING_INTENSITY.set(params["BANDING_INTENSITY"])
        self.BAND_APPLY_PROB.set(params["BAND_APPLY_PROB"])
    ...
    def randomize_parameters(self):
        ...
        self.BANDING_INTENSITY.set(random.uniform(0.0, 1.0))
        self.BAND_APPLY_PROB.set(random.uniform(0.0, 1.0))

    def disable_parameters(self, disable=True):
        state = "disabled" if disable else "!disabled"
        for param in [
            "NOISE_INTENSITY", "ROTATION_RANGE", "SCALE_RANGE",
            "FLIP_PROBABILITY", "BRIGHTNESS_RANGE", "CONTRAST_RANGE",
            "HUE_SHIFT", "BLUR_INTENSITY", "EDGE_DETECTION_PROB",
            "FRAME_SKIP_PROB", "BANDING_INTENSITY", "BAND_APPLY_PROB"
        ]:
    ...
    def load_video_info(self, filepath):
        ...
        if fps > 1e-3 and frame_count > 0:
            ...
            # Try AVI ratio seek to end and read POS_MSEC
            cap.set(cv2.CAP_PROP_POS_AVI_RATIO, 1)
            duration_ms = cap.get(cv2.CAP_PROP_POS_MSEC)
            if duration_ms and duration_ms > 0:
                duration = duration_ms / 1000.0
            else:
                # Fallback: iterate frames quickly with grab (no decode)
                total_frames = 0
                ...
                if fps > 1e-3:
                    ...
                    # assume 30 fps as last resort
                    duration = total_frames / 30.0
        # Reset capture to start for further operations
        cap.release()

        # Open again to get dimensions cleanly (cheap compared to reading frames)
        cap = cv2.VideoCapture(filepath)
        self.video_info = {
            "fps": fps if fps > 1e-3 else cap.get(cv2.CAP_PROP_FPS) or 30,
            "duration": duration,
        ...
        for slider in [self.start_slider, self.end_slider]:
            ...
            if slider is self.start_slider:
                slider["var"].set(0)
            else:
                slider["var"].set(duration)

        # Update duration label
        if hasattr(self, "duration_label"):
            self.duration_label.config(text=f"Video Duration: {duration:.2f} s")
        else:
            self.duration_label = ttk.Label(self.root, text=f"Video Duration: {duration:.2f} s")
            self.duration_label.grid(row=4, column=0, padx=10, pady=(0,10), sticky="w")
    ...
    def process_frame(self, frame, params):
        """Process a single frame using all selected effects."""
        # Optionally skip frame
        ...
        frame = self.apply_noise(frame, params)
        frame = self.apply_filters(frame, params)
        frame = self.apply_banding_effect(
            frame,
            intensity=params["BANDING_INTENSITY"],
            prob=params["BAND_APPLY_PROB"],
        )
        return frame
    ...
    def apply_banding_effect(self, frame, intensity, prob=None):
        """Apply random vertical and horizontal banding effect.
        Args:
            frame: input BGR image.
            intensity: blending factor between 0 and 1.
            prob: probability to apply effect to this frame.
        """
        ...
        for _ in range(random.randint(2, 8)):
            ...
            bw = random.randint(max(3, w // 100), max(6, w // 10))
            color_shift = np.random.randint(-50, 50, size=(1, 1, 3))
            banded[:, x:x+bw] = np.clip(banded[:, x:x+bw] + color_shift, 0, 255)
        # Horizontal bands
        for _ in range(random.randint(2, 8)):
            y = random.randint(0, h - 1)
            bh = random.randint(max(3, h // 100), max(6, h // 10))
            color_shift = np.random.randint(-50, 50, size=(1, 1, 3))
            banded[y:y+bh, :] = np.clip(banded[y:y+bh, :] + color_shift, 0, 255)
        return cv2.addWeighted(frame, 1 - intensity, banded, intensity, 0)
